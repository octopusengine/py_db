...
Zachován je koncept aktivní tabulky. Po USE customers pracují LIST, FIND,
LOCATE, INSERT, UPDATE, REPLACE, DELETE, STRUCT, MODIF a EXPORT s tabulkou
customers.
...
| LIST / SELECT | Vypíše řádky, případně jen vybrané sloupce; umí WHERE, ORDER BY, LIMIT/OFFSET a PAGE/SIZE. |
| FIND / LOCATE FOR | Vypíše první řádek vyhovující podmínce nad aktivní tabulkou. |
| STRUCT | Vypíše sloupce, SQLite typy a primární klíč. |
| INSERT nebo INSE | Vloží řádek do aktivní tabulky. |
| UPDATE | Změní vybrané sloupce pro řádky odpovídající podmínce WHERE. |
| REPLACE | dBASE-style zápis změny pomocí WITH a FOR. |
...
- Nejsou indexové soubory NDX, SET ORDER, SEEK, GO TOP, SKIP ani EOF(). LOCATE
  je zde pouze vyhledání a výpis první shody, bez dBASE ukazatele aktuálního řádku.
...
- Nejsou implementovány APPEND, BROWSE, EDIT, COUNT, SUM, AVERAGE, COPY, JOIN
  ani REPORT FORM. REPLACE a FIND/LOCATE existují v omezené podobě nad aktivní
  tabulkou.
...
- [x] Přidat LIST WHERE, LIST ORDER BY, LIMIT a stránkování.
- [x] Přidat FIND nebo LOCATE nad aktivní tabulkou.
- [x] Přidat UPDATE nebo dBASE-style REPLACE.
...
- [ ] Navrhnout omezenou podporu APPEND a BROWSE; REPLACE a LOCATE jsou hotové v základním rozsahu.
...
- [x] Přidat automatické testy příkazů, chybných vstupů a JSON definic.
- [x] Doplnit návratové kódy CLI a strojově čitelný výstup pro --list.
- [x] Přidat --no-debug nebo přesunout debug výpisy pod vlastní přepínač.
